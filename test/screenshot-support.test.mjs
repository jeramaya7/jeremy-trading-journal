...
test('risk fields and JSON persistence hooks are rendered', () => {
  assertIncludes(source, 'Stop Loss', 'The Stop Loss field is rendered.');
  assertIncludes(source, 'Account Size', 'The Account Size field is rendered.');
  assertIncludes(source, 'Risk %', 'The Risk % field is rendered.');
  assertIncludes(source, "stopLoss: optionalNumber(formData.get('stopLoss'))", 'Stop Loss is saved on the trade object for JSON export.');
  assertIncludes(source, "accountSize: optionalNumber(formData.get('accountSize'))", 'Account Size is saved on the trade object for JSON export.');
  assertIncludes(source, 'nextTrade.riskPercent = calculateRiskPercent(nextTrade);', 'Risk % is saved on the trade object for JSON export.');
});
