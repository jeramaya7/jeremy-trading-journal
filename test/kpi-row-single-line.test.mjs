...
test('Trading Mode KPI strip (6 cards) uses a 6-column desktop grid so CE stays on the same row', () => {
  ...
  assertIncludes(
    styles,
    '.stats-grid.trading-today-kpi-strip {\n    gap: clamp(0.65rem, 1vw, 0.9rem);\n    grid-template-columns: repeat(6, minmax(0, 1fr));\n  }',
    'The Trading Mode KPI strip should use a 6-column desktop grid, one column per card, so all six fit on a single row.',
  );
});

test('Dashboard header KPI strip (5 cards) uses a 5-column desktop grid so CE stays on the same row', () => {
...
test('both single-row overrides only apply at desktop width, so mobile/tablet stacking is unaffected', () => {
  ...
  const sixColIndex = styles.indexOf('grid-template-columns: repeat(6, minmax(0, 1fr));');
  ...
  // class alongside their strip-specific class.
