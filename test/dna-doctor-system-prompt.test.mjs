...
test('the DNA Doctor system prompt uses the new evidence-based coaching style', () => {
  ...
  assert.ok(prompt.includes('Average Loser greater than Average Winner, low Average Winner, high Win Rate, large recorded risk, Protected %, trail-stop use, defensive management, and CE are not automatically good or bad by themselves.'), 'Individual metrics should not be judged in isolation.');
  ...
  assert.ok(prompt.includes('Biggest Weakness must identify a material problem demonstrated by the combined data; do not select a metric just because it is the least attractive number.'), 'Biggest Weakness should not pick the weakest-looking isolated metric.');
