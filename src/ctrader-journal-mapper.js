...
function getCtraderSymbolMetadataForDeal(deal, openingDeal = null, options = {}) {
  const symbolId = deal?.symbolId
    ?? deal?.closePositionDetail?.symbolId
    ?? openingDeal?.symbolId
    ?? openingDeal?.closePositionDetail?.symbolId;
...
function getCtraderDealSymbol(deal, openingDeal = null, symbolMetadata = null) {
  ...
  const brokerSymbolName = symbolCandidates.find((candidate) => !isNumericSymbolIdentifier(candidate));
  if (brokerSymbolName) {
    return String(brokerSymbolName);
  }

  return symbolCandidates.find((candidate) => candidate !== undefined && candidate !== null && candidate !== '')
    || (deal.symbolId !== undefined ? String(deal.symbolId) : null)
    || (deal.closePositionDetail?.symbolId !== undefined ? String(deal.closePositionDetail.symbolId) : null)
    || (openingDeal?.symbolId !== undefined ? String(openingDeal.symbolId) : null)
    || (openingDeal?.closePositionDetail?.symbolId !== undefined ? String(openingDeal.closePositionDetail.symbolId) : null)
    || 'Unknown';
}

function isNumericSymbolIdentifier(value) {
